...
public final class SortUtils {
    ...
    static <T> Comparator<T> genComparator(Class<T> type) {
        ...
        try {
            for (Field sortField : sortFields) {
                String comparatorName = sortField.getAnnotation(Sort.class).comparator();
            ...
            return (o1, o2) -> {
                ...
                try {
                    ...
                    for (Field sortField : sortFields) {
                        ...
                        temp = compareBaseValue(v1, v2, sortField.getAnnotation(Sort.class).value());
                        if (0 != temp) {
                            break;
                        }
                    }
    ...
    static <T> Comparator<T> genComparator(TreeNodeClassInfo classInfo) {
        ...
        classInfo.getFieldInfoList().stream().filter(treeFieldInfo -> null != treeFieldInfo.getSort()).map(
                treeFieldInfo -> {
                    try {
                        String comparatorName = treeFieldInfo.getSort().comparator();
                        if (StrUtil.isNotBlank(comparatorName)) {
                            Class comparatorClass = Class.forName(comparatorName);
                            Comparator comparator = (Comparator) comparatorClass.newInstance();
            ...
            try {
                for (Map.Entry<TreeFieldInfo, Comparator> entry :
                    ...
                    if (null != comparator) {
                        ...
                        compareRes = compareBaseValue(v1, v2, entry.getKey().getSort().value());
                    }
                    if (0 != compareRes) {
                        break;
                    }
    ...
    private static List<Field> getSortField(Class c) {
        ...
        for (Field field :
                fields) {
            sort = field.getAnnotation(Sort.class);
            if (null != sort) {
                res.add(field);
            }
        }
