...
import com.tchuhu.common.core.util.StringUtil;
import lombok.Data;
 ...
 * @date 2018/1/31
 * @time 19:03
 * @Path cn.phxg.common.utils.TreeUtils
 */
public class TreeUtils {
     ...
     */
    private static void setParentLeaf(TreeNode currentNode, LinkedHashMap<String, Object> keyNodeMap){
     ...
     * @param keyNodeMap
     */
    private static void mapNodeHandle(Object nodeObj, LinkedHashMap<String, Object> keyNodeMap){
        if (null == nodeObj) {
            return;
        }
    ...
    private static JSONArray genCleanTree(List<? extends TreeExtend> treeExtends, boolean autoSort) {
        ...
        if (!CollectionUtils.isEmpty(treeExtends)) {
            // 将节点列表散列为key->node结构
            LinkedHashMap<String, Object> keyNodeMap = new LinkedHashMap<>(1);
            // 根据结果集构造节点列表（存入散列表）
            ...
            for (Object current: keyNodeMap.values()) {
                ...
                if (current instanceof LinkedHashMap) {
                    for (TreeNode currentNode: ((LinkedHashMap<String, TreeNode>) current).values()) {
                        //根节点
                        if (StrUtil.isBlank(currentNode.getParentId()) || null == keyNodeMap.get(currentNode.getParentId())) {
                ...
                //根节点
                if (StrUtil.isBlank(currentNode.getParentId())
     ...
     * @param keyNodeMap
     */
    private static void nodeListToMap(List<? extends TreeExtend> treeExtends, HashMap<Class, TreeNodeClassInfo> treeClassCache, LinkedHashMap<String, Object> keyNodeMap) {
    ...
    private static JSONArray genTreeExtendList(List<? extends TreeExtend> treeExtends, boolean autoSort) {
        ...
        if (!CollectionUtils.isEmpty(treeExtends)) {
            // 节点列表（散列表，用于临时存储节点对象）
            LinkedHashMap<String, Object> nodeList = new LinkedHashMap<>(1);
            // 根节点
            TreeNode root;
            // 根据结果集构造节点列表（存入散列表）
    ...
    private static JSONObject genTreeExtend(List<? extends TreeExtend> treeExtends, boolean autoSort) {
        ...
        if (null != dataList && 0 < dataList.size()) {
            ...
            TreeNode root = null;
            // 节点列表（散列表，用于临时存储节点对象）
            LinkedHashMap<String, TreeNode> nodeList = dataList.stream()
                    .collect(
                            Collectors.toMap(
                                    TreeNode::getId,
                                    node -> node,
        ...
        if (null == classInfo) {
            ...
            fields.forEach(
                    field -> {
                        ...
                        treeFieldInfo.setSort(field.getAnnotation(Sort.class));
    ...
    private static void nodeDefaultFieldHandle(TreeProperty treeProp, TreeNode node, Field f, TreeExtend treeExtend) throws IllegalAccessException{
        if (null != treeProp) {
            ...
            if (TreeField.ID.equals(treeProp.value())) {
                node.setId(StringUtil.getString(f.get(treeExtend)));
            }
            //节点父id
            else if (TreeField.PARENTID.equals(treeProp.value())) {
                node.setParentId(StringUtil.getString(f.get(treeExtend)));
            }
            //节点展示名称
            else if (TreeField.NAME.equals(treeProp.value())) {
                node.setName(StringUtil.getString(f.get(treeExtend)));
            }
            //是否叶子节点
